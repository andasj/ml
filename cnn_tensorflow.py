...
if __name__ == '__main__':
    ...
    checkpoint = ModelCheckpoint('models/model_cnn_mnist.keras', monitor='val_accuracy', save_best_only=True)
    history = model.fit(dataset['x_train'], dataset['y_train'], epochs=10, validation_data=(dataset['x_val'], dataset['y_val']), callbacks=[checkpoint])
    model_evaluate(model, dataset, history)
    predict = model_predict(model, dataset['x_test'])

    # Prediction
    model_pred = load_model('models/model_cnn_mnist.keras')
    image = cv2.imread('images/example_mnist.png', 0)
